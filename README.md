# AI Pick Vault

**Smart deals. AI curated. Real value.**

Grok-powered product picks for gig workers, DIYers, van lifers, and independent hustlers.

## Live site

**https://bamtec70.github.io/aipickvault/**

- TikTok: [@aipickvault](https://www.tiktok.com/@aipickvault)

## Local

```powershell
cd C:\Users\bamte\aipickvault
start index.html
```

## Amazon Associates tag

Edit **one line** near the bottom of `index.html`:

```js
const AMAZON_TAG = "yourtag-20";
```

All Shop on Amazon buttons pick it up automatically.

## Affiliate disclosure

As an Amazon Associate we earn from qualifying purchases. Disclosures are shown on the site.

## Stack

Static HTML + Tailwind CDN + Font Awesome. Hosted on GitHub Pages from the `main` branch root.
